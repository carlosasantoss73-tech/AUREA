...
/** Routes communication only; it never grants business authority. */
export class ExecutiveChannelRouter {
  ...
  route(request: ExecutiveChannelRequest): ExecutiveChannelResult {
    ...
    if (!entry || !entry.active) {
      return { decision: "BLOCK", companyId: request.targetCompanyId, traceId: request.traceId, reason: "COMPANY_COMMUNICATION_CHANNEL_UNAVAILABLE" };
    }

    if (request.requestedChannel === "EXECUTIVE_DIRECT") {
      if (request.requesterType !== "PRESIDENT") {
        return { decision: "BLOCK", companyId: request.targetCompanyId, traceId: request.traceId, reason: "EXECUTIVE_DIRECT_CHANNEL_REQUIRES_PRESIDENT" };
      }
      return { decision: "ROUTE", channel: "EXECUTIVE_DIRECT", targetAgentId: entry.ceoAgentId, companyId: request.targetCompanyId, traceId: request.traceId };
    }

    return { decision: "ROUTE", channel: "CUSTOMER_SERVICE", targetAgentId: entry.customerServiceAgentId, companyId: request.targetCompanyId, traceId: request.traceId };
  }
}
