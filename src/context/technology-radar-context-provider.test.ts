...
describe("Technology Radar continuity provider", () => {
  it("recovers video candidates from the durable B11 registry", async () => {
    const context = await technologyRadarContextProvider.retrieve({
      projectId: "aurea",
      query: "herramientas de video que trabajamos esta semana",
      traceId: "trace-test",
    });
  ...
  it("recovers voice tooling separately", async () => {
    const context = await technologyRadarContextProvider.retrieve({
      projectId: "aurea",
      query: "herramientas de voz TTS",
      traceId: "trace-test",
    });
