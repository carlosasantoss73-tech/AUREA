 ...
 *
 * Purpose: prevent the agent from answering historical/continuity-sensitive
 * requests from local conversational recall alone. When a query contains a
 * continuity signal, the gate performs a governed READ against the registered
 * knowledge provider before the agent/tool path is allowed to continue.
 */
...
export interface ContextProvider {
  retrieve(input: { projectId: string; query: string; traceId: string }): Promise<RetrievedContext>;
}
...
/** Returns true when answering without historical retrieval would be unsafe or unreliable. */
...
export class ContextRetrievalGate {
  ...
  async retrieve(request: ContextRetrievalRequest): Promise<ContextRetrievalResult> {
    ...
    if (!requiresHistoricalContext(request.query)) {
      return { traceId, status: "NOT_NEEDED", reason: "NO_CONTINUITY_SIGNAL" };
    }
    ...
    if (!["ALLOW", "ALLOW_WITH_LIMITS"].includes(permission.decision)) {
      return { traceId, status: "BLOCKED", reason: permission.reason };
    }

    const context = await this.provider.retrieve({
      projectId: request.projectId,
      query: request.query,
      traceId,
    });
    if (!context.citations.length && !context.facts.length) {
      return { traceId, status: "EMPTY", reason: "HISTORICAL_CONTEXT_NOT_FOUND" };
    }
