...
describe("AUREA memory ingestion", () => {
  it("creates evidence and can be retrieved", async () => {
    const records: any[] = [];
    const store = { records, search: () => [], upsert: async (record: any) => records.push(record) };
  ...
  it("is idempotent for the same evidence", async () => {
    const records: any[] = [];
    const store = { records, search: () => [], upsert: async (record: any) => { const i = records.findIndex(r => r.id === record.id); if (i >= 0) records[i] = record; else records.push(record); } };
