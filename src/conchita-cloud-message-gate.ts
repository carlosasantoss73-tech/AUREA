...
/**
 * Server-side transport boundary: the browser supplies only a session handle
 * and message. Identity is established by the authenticated session, never
 * by browser-controlled userId data.
 */
export class ConchitaCloudMessageGate {
  constructor(
    private readonly authenticator: ConchitaCloudSessionAuthenticator,
    private readonly gateway: Pick<ConchitaPersonalV0Gateway, "sendAuthenticatedMessage">,
  ) {}

  async handle(request: ConchitaPwaRequest): Promise<ConchitaCloudMessageGateResult> {
    const sessionId = request.sessionId.trim();
    if (!sessionId || !request.message.trim() || !request.clientRequestId.trim()) {
      return { accepted: false, reason: "INVALID_TRANSPORT_REQUEST" };
    }
