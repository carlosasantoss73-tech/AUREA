...
interface Env { CONCHITA_SESSIONS: ConchitaKvNamespace; CONCHITA_ANTHROPIC_MODEL: string; ANTHROPIC_API_KEY: string; CONCHITA_PILOT_USER_ID: string; CONCHITA_PILOT_BOOTSTRAP_TOKEN: string; CONCHITA_ALLOWED_ORIGIN: string; }
...
function runtimeHealth(env: Env): { status: 'HEALTHY' | 'BLOCKED'; checks: Record<string, boolean>; blockers: string[] } {
  const checks = { kv: Boolean(env.CONCHITA_SESSIONS), providerCredentials: Boolean(env.ANTHROPIC_API_KEY), providerModel: Boolean(env.CONCHITA_ANTHROPIC_MODEL), pilotUser: Boolean(env.CONCHITA_PILOT_USER_ID), bootstrapAuth: Boolean(env.CONCHITA_PILOT_BOOTSTRAP_TOKEN), allowedOrigin: Boolean(env.CONCHITA_ALLOWED_ORIGIN) };
...
function makePilotMessageHandler(env: Env): (request: Request) => Promise<Response> {
  ...
  platform.register({ manifest: { integrationId: 'conchita-cloudflare-edge', componentId: 'conchita-cloudflare-worker', version: '1', boundaries: ['OPERATIONS', 'EXECUTION', 'PERMISSION', 'WORK_CELL'], requiredCapabilities: ['conchita.chat'], requiredConfiguration: ['CONCHITA_SESSIONS', 'ANTHROPIC_API_KEY', 'CONCHITA_PILOT_USER_ID'], healthChecks: ['https', 'kv', 'provider'], rollbackPlan: 'Disable the Worker deployment and preserve KV state.' }, inspect: () => ({ integrationId: 'conchita-cloudflare-edge', status: env.ANTHROPIC_API_KEY && env.CONCHITA_PILOT_USER_ID ? 'HEALTHY' : 'BLOCKED', checkedAt: new Date().toISOString(), checks: { healthy: Boolean(env.ANTHROPIC_API_KEY && env.CONCHITA_PILOT_USER_ID) }, evidence: ['CLOUDFLARE_WORKER', 'KV_SESSION_BOUNDARY', 'SECRET_PROVIDER_BOUNDARY'], blockers: env.ANTHROPIC_API_KEY && env.CONCHITA_PILOT_USER_ID ? [] : ['CLOUD_CONFIGURATION_INCOMPLETE'] }) });
