...
export class ConchitaPersonalV0Gateway implements ConchitaPersonalGateway {
  constructor(private readonly handler: ConchitaPersonalRequestHandler, private readonly sessions: ConchitaSessionStore, private readonly now: () => string = () => new Date().toISOString(), private readonly id: () => string = () => crypto.randomUUID()) {}
  ...
  async sendAuthenticatedMessage(principal: ConchitaAuthenticatedPrincipal | undefined, request: ConchitaMessageRequest): Promise<ConchitaMessageResponse> {
    ...
    if (!session || !this.sessions.isActive(request.sessionId)) return this.blocked(request, ['SESSION_NOT_ACTIVE']);
  ...
  async closeSession(sessionId: string): Promise<void> { this.sessions.revoke(sessionId); }
